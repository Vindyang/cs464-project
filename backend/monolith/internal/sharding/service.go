...
func (s *service) ChunkFile(data []byte, chunkSize int64) ([][]byte, error) {
	if chunkSize <= 0 {
...
func (s *service) EncodeChunk(chunkData []byte, k, n int) ([][]byte, error) {
	if k <= 0 || n <= 0 || k > n {
	...
	for index := 0; index < k; index++ {
		...
		if start < len(chunkData) {
			copy(shards[index], chunkData[start:end])
		}
	}
...
func (s *service) DecodeChunk(shards [][]byte, k, n int) ([]byte, error) {
	if k <= 0 || n <= 0 || k > n {
